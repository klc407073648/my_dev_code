...
int main(int argc, const char * argv[])
{
    ...
    while(1)
    {
    	...
    	FD_SET(STDIN_FILENO, &server_fd_set); /* 构造select监控文件描述符集 */
		...
		//服务器端socket
        FD_SET(server_sock_fd, &server_fd_set);/* 构造select监控文件描述符集 */
        ...
        //  监听的最大文件描述符+1  读事件   写事件  异常事件  设置监听时间
        ...
        {
        	//ret 为未状态发生变化的文件描述符的个数，即有数据读入对应文件描述符

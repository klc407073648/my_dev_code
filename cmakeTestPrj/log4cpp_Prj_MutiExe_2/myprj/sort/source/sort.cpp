...
//CREATE_LOGGERMANAGER(SORT);

Sort::Sort()
{
	
	//LoggerManager::getInstance("SORT");
	CREATE_LOGGERMANAGER(SORT);
	...
	for(iter = LoggerManager::getLogger().begin(); iter != LoggerManager::getLogger().end(); iter++)
	{
		...
		if( iter->first == "SORT")
		{
			m_Logger=&iter->second;
			logError(*m_Logger,"Sort Func Test");
			break;
		}
	}
...
Sort::~Sort()
{
	...
	for(iter = LoggerManager::getLogger().begin(); iter != LoggerManager::getLogger().end(); iter++)
	{
		...
		if( iter->first == "SORT")
		{
			logError(iter->second,"Sort Func END");
			break;
		}
	}
	logError(*m_Logger,"Sort Func END11");
	logError(*m_Logger,"Sort Func END22");
	cout<<"size:"<<LoggerManager::getLogger().size()<<endl;
}

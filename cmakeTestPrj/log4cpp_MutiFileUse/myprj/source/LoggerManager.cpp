...
LoggerManager::LoggerManager(): 
    category_ref_(log4cpp::Category::getRoot()) {
		//    m_ref_(log4cpp::Category::getRoot()),category_ref_(m_ref_.getInstance(m_name))
	//category_ref_=m_ref_.getInstance(m_name);
    //自定义输出格式
    log4cpp::PatternLayout *pattern_one = new log4cpp::PatternLayout;
    pattern_one->setConversionPattern("%d{%Y-%m-%d,%H:%M:%S}: %p :%m%n");

	//pattern_one->setConversionPattern("%d{%Y-%m-%d,%H:%M:%S}: %p %c %x:%m%n");
	//%c 输出日志信息所属的类目，通常就是所在类的全名
    log4cpp::PatternLayout *pattern_two = new log4cpp::PatternLayout;
    pattern_two->setConversionPattern("%d{%Y-%m-%d,%H:%M:%S}: %p :%m%n");
